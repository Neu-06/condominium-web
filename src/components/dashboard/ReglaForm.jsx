...
export default function ReglaForm({ initialData=null, areas=[], onSubmit, onCancel, loading=false }) {
  const editMode = !!initialData;
  const [form,setForm]=useState({ nombre:"", descripcion:"", activo:true, areasSeleccionadas:[] });
  ...
  useEffect(()=>{
    if(initialData){
      setForm({
        ...
        areasSeleccionadas: (initialData.areas_ids || initialData.areas_ids === 0)
          ? (initialData.areas_ids || [])
          : (initialData.areas || []).map(a=>a.id)
      });
    } else {
      setForm({ nombre:"", descripcion:"", activo:true, areasSeleccionadas:[] });
    }
  },[initialData]);
    ...
    <div className="w-full flex justify-center px-3">
      <form onSubmit={submit}
        ...
        <div className="flex flex-col md:flex-row justify-between gap-4">
          <h2 className="text-2xl font-bold text-gray-800">{editMode?"Editar Regla":"Nueva Regla"}</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <Field label="Descripción">
            <textarea className="input-base h-32 resize-y"
          ...
          <Field label="Áreas (opcional)">
            <div className="max-h-40 overflow-auto border rounded-2xl p-2 flex flex-col gap-1">
              {areas.length === 0 && <span className="text-xs text-gray-400 px-1">No hay áreas</span>}
              {areas.map(a=>(
                <label key={a.id} className="flex items-center gap-2 text-xs px-2 py-1 rounded hover:bg-gray-50">
                  <input
                    type="checkbox"
                    disabled={loading}
